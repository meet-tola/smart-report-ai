...
export function handleLayoutChange(editor: MyEditor, type: string): void {
  ...
  if (PARENT_CHILD_RELATIONSHIP[element.type as keyof typeof PARENT_CHILD_RELATIONSHIP]?.child) {
    editor.tf.withoutNormalizing(() => {
      editor.tf.setNodes({ type }, { at: editor.api.findPath(element) });
      element.children.forEach((child: any) => {
        editor.tf.setNodes(
          { type: PARENT_CHILD_RELATIONSHIP[type as keyof typeof PARENT_CHILD_RELATIONSHIP]?.child },
