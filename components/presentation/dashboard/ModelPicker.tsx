...
export function ModelPicker({
  ...
  const ollamaModels = localModels.filter(
    (model: { provider: string; }) => model.provider === "ollama",
  );
  const lmStudioModels = localModels.filter(
    (model: { provider: string; }) => model.provider === "lmstudio",
  );
  ...
  const getCurrentModelOption = () => {
    ...
    const localModel = localModels.find((model: { id: string; }) => model.id === currentValue);
    if (localModel) {
      return {
    ...
    const downloadableModel = downloadableModels.find(
      (model: { id: string; }) => model.id === currentValue,
    );
        ...
        <SelectContent>
          ...
          {showDownloadable && downloadableOllamaModels.length > 0 && (
            <SelectGroup>
              ...
              {downloadableOllamaModels.map((model: any) => {
