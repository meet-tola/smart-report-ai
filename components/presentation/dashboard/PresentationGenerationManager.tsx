...
export function PresentationGenerationManager() {
  ...
  // Function to get assistant text content from parts
  const getAssistantTextContent = (parts: any[]): string => {
    return parts
  ...
  const processMessages = (messages: typeof outlineMessages): void => {
    ...
    if (lastMessage.role === "assistant") {
      const assistantText = getAssistantTextContent(lastMessage.parts);

      if (assistantText) {
        // Extract <think> content from assistant message and keep only the remainder for parsing
        const thinkingExtract = extractThinking(assistantText);
