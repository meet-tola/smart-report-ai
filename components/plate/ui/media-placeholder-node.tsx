...
// ---- FIX: safe minimal type for use-file-picker ----
...
export const PlaceholderElement = withHOC(
  ...
  function PlaceholderElement(props: PlateElementProps<TPlaceholderElement>) {
    ...
    const { openFilePicker } = useFilePicker({
      ...
      // ---- FIX: use MinimalFilePickerData for safe typing ----
      onFilesSelected: ({ plainFiles }: MinimalFilePickerData) => {
        if (!plainFiles?.length) return;

        const firstFile = plainFiles[0];
        const restFiles = plainFiles.slice(1);

        replaceCurrentPlaceholder(firstFile);

        function filesArrayToFileList(files: File[]): FileList {
        ...
        if (restFiles.length > 0) {
          editor
            ...
            .insert.media(filesArrayToFileList(restFiles));
        }
      },
    });
    ...
    // React dev mode calls effects twice
