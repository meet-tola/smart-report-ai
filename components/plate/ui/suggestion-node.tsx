...
export function SuggestionLeaf(props: PlateLeafProps<TSuggestionText>) {
  ...
  const filteredDataList = dataList.filter(
    (data): data is TSuggestionData => data.type === "insert" || data.type === "remove"
  );
  const hasRemove = filteredDataList.some((data) => data.type === "remove");
  const hasActive = filteredDataList.some((data) => data.id === activeSuggestionId);
  const hasHover = filteredDataList.some((data) => data.id === hoverSuggestionId);
...
export const SuggestionLineBreak: RenderNodeWrapper<SuggestionConfig> = ({
  ...
  const suggestionData = element.suggestion as TSuggestionData & { isLineBreak?: boolean };
...
function SuggestionLineBreakContent({
  ...
  suggestionData: TSuggestionData & { isLineBreak?: boolean };
}) {
