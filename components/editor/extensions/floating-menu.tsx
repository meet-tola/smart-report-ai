...
export function TipTapFloatingMenu({ editor }: { editor: Editor }) {
  ...
  useEffect(() => {
    ...
    // Resolve possible shapes for editor.options.element:
    // - HTMLElement / Element
    // - { mount: HTMLElement }
    // - (editor: HTMLElement) => void  (no DOM element to attach to)
    ...
    if (
      ...
      element &&
      "mount" in element &&
      (element as any).mount instanceof HTMLElement
    ) {
      editorElement = (element as any).mount as HTMLElement;
